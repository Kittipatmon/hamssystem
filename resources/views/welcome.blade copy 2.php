...
@section('content')
    ...
    <div class="max-w-7xl mx-auto space-y-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <!-- Policy Section -->
            <div class="bg-white rounded-[2.5rem] p-6 shadow-sm border border-slate-100/50">
                <div class="flex items-start gap-4 mb-8">
                    ...
                    <div>
                        <h2 class="text-xl font-bold text-blue-900 leading-tight">หมวดหมู่นโยบาย</h2>
                        <p class="text-xs text-slate-400 mt-1">ข้อกำหนดและแนวทางปฏิบัติขององค์กร</p>
                    </div>
                </div>

                <div class="space-y-4">
                    @forelse($policies as $policy)
                        <div class="bg-blue-50/50 rounded-2xl p-5 border border-blue-100/50 hover:border-blue-200 transition-all group">
                            <div class="flex gap-4">
                                <div class="w-8 h-8 rounded-full bg-blue-600 text-white flex items-center justify-center font-bold text-xs shrink-0 mt-0.5">
                                    {{ $loop->iteration }}
                                </div>
                                <div class="flex-1">
                                    <h3 class="text-blue-900 font-bold mb-1.5">{{ $policy->title }}</h3>
                                    <p class="text-[13px] text-blue-800/70 leading-relaxed font-medium">
                                        {{ $policy->content }}
                                    </p>
                                </div>
                            </div>
                    ...
                    @empty
                        <p class="text-center text-slate-400 py-10 italic">ยังไม่มีข้อมูลนโยบาย</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="bg-white rounded-[2.5rem] p-6 shadow-sm border border-slate-100/50">
                <div class="flex items-start gap-4 mb-8">
                    <div class="w-14 h-14 rounded-2xl bg-emerald-50 flex items-center justify-center flex-shrink-0">
                        <i class="fa-solid fa-info text-emerald-600 text-2xl"></i>
                    </div>
                    <div>
                        <h2 class="text-xl font-bold text-emerald-900 leading-tight">หมวดหมู่การดำเนินงาน</h2>
                        <p class="text-xs text-slate-400 mt-1">ขั้นตอนและแนวทางการปฏิบัติงาน</p>
                    </div>
                </div>

                <div class="space-y-4">
                    @forelse($operations as $op)
                        <div class="bg-emerald-50/50 rounded-2xl p-5 border border-emerald-100/50 hover:border-emerald-200 transition-all group">
                            <div class="flex gap-4">
                                <div class="w-8 h-8 rounded-full bg-emerald-600 text-white flex items-center justify-center font-bold text-xs shrink-0 mt-0.5">
                                    {{ $loop->iteration }}
                                </div>
                                <div class="flex-1">
                                    <h3 class="text-emerald-900 font-bold mb-1.5">{{ $op->title }}</h3>
                                    <p class="text-[13px] text-emerald-800/70 leading-relaxed font-medium">
                                        {{ $op->content }}
                                    </p>
                                </div>
                            </div>
                    ...
                    @empty
                        <p class="text-center text-slate-400 py-10 italic">ยังไม่มีข้อมูลการดำเนินงาน</p>
                    @endforelse
                </div>
            </div>
        </div>
        ...
        {{-- ============ NEWS SECTION ============ --}}
        <div>
            <div id="news" class="flex items-center justify-between mb-1 scroll-mt-20">
