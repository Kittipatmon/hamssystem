    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4">
            ...
            <div>
                ...
                <div class="text-2xl font-black text-slate-800">{{ number_format($items_types->count()) }} <span class="text-xs font-normal text-slate-400 ml-1">ประเภท</span></div>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex items-center gap-4">
            ...
            <div>
                ...
                <div class="text-2xl font-black text-emerald-600">{{ number_format($activeCount) }} <span class="text-xs font-normal text-slate-400 ml-1">ประเภท</span></div>
            </div>
        </div>
    </div>
                ...
                <table id="typeTable" class="w-full text-left border-collapse min-w-[800px]">
                    <thead>
                        <tr class="bg-slate-50/50">
                            <th class="px-6 py-4 text-[12px] font-black text-slate-400 uppercase tracking-widest rounded-l-2xl text-center">#</th>
                            <th class="px-6 py-4 text-[12px] font-black text-slate-400 uppercase tracking-widest">ชื่อประเภท</th>
                            <th class="px-6 py-4 text-[12px] font-black text-slate-400 uppercase tracking-widest">คำอธิบาย</th>
                            <th class="px-6 py-4 text-[12px] font-black text-slate-400 uppercase tracking-widest text-center">สถานะ</th>
                    ...
                    <tbody class="divide-y divide-slate-50">
                        ...
                        <tr class="hover:bg-red-50/20 transition-all duration-200">
                            ...
                            <td class="px-6 py-4">
                                <p class="text-[13px] text-slate-500 truncate max-w-[300px]">{{ $type->description ?: '-' }}</p>
                            </td>
                            <td class="px-6 py-4 text-center">
                                <form action="{{ route('items_type.toggleStatus', $type->item_type_id) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="submit" class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full {{ $type->status ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600' }} text-[11px] font-black uppercase transition-all hover:scale-105 active:scale-95">
        ...
        <div class="lg:hidden grid grid-cols-1 gap-4">
            ...
            <div class="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 space-y-4">
                <div class="flex items-center justify-between">
                    <span class="text-[10px] font-bold text-slate-400 bg-slate-50 px-2 py-0.5 rounded-lg border border-slate-100 text-center min-w-[24px]">#{{ $loop->iteration }}</span>
                    <form action="{{ route('items_type.toggleStatus', $type->item_type_id) }}" method="POST" class="inline">
                        @csrf
                        <button type="submit" class="px-3 py-1 rounded-full {{ $type->status ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600' }} text-[10px] font-black uppercase">
                            {{ $type->status ? 'เปิดใช้งาน' : 'ปิดใช้งาน' }}
                        </button>
                    </form>
                </div>
                ...
                <div class="flex gap-2 pt-2">
                    <button 
                        ...
                        data-name="{{ $type->name }}"
                        data-description="{{ $type->description ?: '' }}"
                        class="flex-1 h-12 flex items-center justify-center bg-slate-800 text-white font-bold rounded-2xl shadow-lg shadow-slate-100"
                    >แก้ไข</button>
                    <button 
                        data-delete
                        data-id="{{ $type->item_type_id }}"
                        data-name="{{ $type->name }}"
                        class="w-12 h-12 flex items-center justify-center bg-white border-2 border-red-50 text-red-500 rounded-2xl shadow-sm"
...
<div id="modalCreate" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
    ...
    <div class="relative bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-zoom-in">
        ...
        <form action="{{ route('items_type.store') }}" method="POST" class="p-8 space-y-6">
            @csrf
            <div class="space-y-2">
                <label class="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">ชื่อเรียกประเภท *</label>
                <input type="text" name="name" required placeholder="เช่น อุปกรณ์สำนักงาน" class="w-full h-14 px-6 rounded-2xl border border-slate-100 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-red-50 focus:border-red-200 transition-all font-bold text-slate-700"/>
            </div>
            <div class="space-y-2">
                <label class="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">รายละเอียดเพิ่มเติม</label>
                <textarea name="description" rows="3" placeholder="ระบุรายละเอียดสั้นๆ..." class="w-full p-6 rounded-2xl border border-slate-100 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-red-50 focus:border-red-200 transition-all font-medium text-slate-600 leading-relaxed"></textarea>
            </div>
            <div class="flex flex-col sm:flex-row gap-3 pt-4">
                <button type="button" data-close class="order-2 sm:order-1 flex-1 h-14 rounded-2xl text-slate-400 font-bold hover:bg-slate-50 transition-colors">ยกเลิก</button>
                <button type="submit" class="order-1 sm:order-2 flex-[2] h-14 bg-red-600 rounded-2xl text-white font-black shadow-lg shadow-red-100 hover:bg-red-700 transition-all active:scale-95">บันทึกข้อมูล</button>
            </div>
        </form>
    </div>
</div>
...
<div id="modalEdit" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
    ...
    <div class="relative bg-white w-full max-w-md rounded-[2.5rem] shadow-2xl overflow-hidden animate-zoom-in">
        ...
        <form id="formEdit" method="POST" class="p-8 space-y-6">
            ...
            <div class="space-y-2">
                <label class="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">ชื่อเรียกประเภท *</label>
                <input id="editName" type="text" name="name" required class="w-full h-14 px-6 rounded-2xl border border-slate-100 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-slate-100 focus:border-slate-300 transition-all font-bold text-slate-700"/>
            </div>
            <div class="space-y-2">
                <label class="text-[11px] font-black text-slate-400 uppercase tracking-widest ml-1">รายละเอียดเพิ่มเติม</label>
                <textarea id="editDescription" name="description" rows="3" class="w-full p-6 rounded-2xl border border-slate-100 bg-slate-50 focus:bg-white focus:ring-4 focus:ring-slate-100 focus:border-slate-300 transition-all font-medium text-slate-600 leading-relaxed"></textarea>
            </div>
            <div class="flex flex-col sm:flex-row gap-3 pt-4">
                <button type="button" data-close class="order-2 sm:order-1 flex-1 h-14 rounded-2xl text-slate-400 font-bold hover:bg-slate-50 transition-colors">ยกเลิก</button>
                <button type="submit" class="order-1 sm:order-2 flex-[2] h-14 bg-slate-800 rounded-2xl text-white font-black shadow-lg shadow-slate-100 hover:bg-slate-900 transition-all active:scale-95">บันทึกการแก้ไข</button>
            </div>
        </form>
    </div>
</div>
...
<div id="modalDelete" class="fixed inset-0 z-50 hidden items-center justify-center p-4">
    ...
    <div class="relative bg-white w-full max-w-sm rounded-[2.5rem] shadow-2xl overflow-hidden animate-zoom-in">
        <div class="p-10 text-center">
            ...
            <p class="text-sm text-slate-400 font-medium leading-relaxed px-4">คุณแน่ใจว่าต้องการลบประเภทอุปกรณ์ <span id="deleteName" class="text-red-600 font-bold"></span> ออกจากระบบ?</p>
        </div>
        <form id="formDelete" method="POST" class="p-8 bg-slate-50 flex gap-3">
            @csrf @method('DELETE')
            <button type="button" data-close class="flex-1 h-14 rounded-2xl bg-white text-slate-400 font-bold hover:bg-slate-100 border border-slate-100 transition-all">ยกเลิก</button>
            <button type="submit" class="flex-1 h-14 bg-red-600 rounded-2xl text-white font-black shadow-lg shadow-red-100 hover:bg-red-700 transition-all active:scale-95">ลบเลย</button>
        </form>
    </div>
</div>
