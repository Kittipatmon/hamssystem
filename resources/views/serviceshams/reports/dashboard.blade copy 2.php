...
<div class="max-w-[1600px] mx-auto px-4 py-8 space-y-8">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 animate-zoom-in relative overflow-hidden">
        ...
        <div class="flex items-center gap-5 relative">
            ...
            <div>
                <h1 class="text-2xl font-bold text-slate-800 leading-none">รายงานสถิติ (Real-time Dashboard)</h1>
                <p class="text-sm text-slate-400 font-semibold mt-1.5 flex items-center gap-2">
                    <span class="px-2 py-0.5 bg-slate-100 rounded text-slate-600">ANALYTICS ENGINE</span>
                    <span>•</span>
                    <span>วิเคราะห์พฤติกรรมการเบิกและสรุปงบประมาณ</span>
                </p>
            </div>
        </div>
        <div class="flex items-center gap-3 relative">
            <button id="btnResetFilters" type="button" class="px-6 py-3 bg-white border-2 border-slate-100 text-slate-500 font-bold rounded-2xl hover:bg-slate-50 hover:border-slate-200 transition-all active:scale-95 text-xs uppercase">
                <i class="fa-solid fa-rotate-left mr-2"></i> Reset
            </button>
            <button id="btnExportCsv" type="button" class="px-8 py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-2xl shadow-lg shadow-emerald-100 transition-all active:scale-95 text-xs uppercase flex items-center gap-3">
    ...
    <div class="bg-slate-800 p-8 rounded-[2.5rem] shadow-xl border border-slate-700 animate-zoom-in">
        <form id="filterForm" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6">
            <div class="space-y-2">
                <label class="text-[10px] font-bold text-slate-500 uppercase pl-2">จากวันที่ (Date From)</label>
                <input type="date" name="date_from" class="w-full h-12 bg-slate-900 border-none rounded-xl px-4 text-white font-bold text-sm focus:ring-2 focus:ring-red-500 transition-all">
            </div>
            <div class="space-y-2">
                <label class="text-[10px] font-bold text-slate-500 uppercase pl-2">ถึงวันที่ (Date To)</label>
                <input type="date" name="date_to" class="w-full h-12 bg-slate-900 border-none rounded-xl px-4 text-white font-bold text-sm focus:ring-2 focus:ring-red-500 transition-all">
            </div>
            <div class="space-y-2">
                <label class="text-[10px] font-bold text-slate-500 uppercase pl-2">สายงาน (Section)</label>
                <select name="section" class="w-full h-12 bg-slate-900 border-none rounded-xl px-4 text-white font-bold text-sm focus:ring-2 focus:ring-slate-500 appearance-none">
            ...
            <div class="space-y-2">
                <label class="text-[10px] font-bold text-slate-500 uppercase pl-2">ฝ่าย (Division)</label>
                <select name="division" class="w-full h-12 bg-slate-900 border-none rounded-xl px-4 text-white font-bold text-sm focus:ring-2 focus:ring-slate-500 appearance-none" data-cascade="division">
                    <option value="">ทั้งหมด (All)</option>
            ...
            <div class="space-y-2">
                <label class="text-[10px] font-bold text-slate-500 uppercase pl-2">แผนก (Dept)</label>
            ...
            <div class="space-y-2">
                <label class="text-[10px] font-bold text-slate-500 uppercase pl-2">ค้นหา (Search)</label>
                <input type="text" id="searchInput" placeholder="Search items..." class="w-full h-12 bg-slate-900 border-none rounded-xl px-4 text-white font-bold text-sm focus:ring-2 focus:ring-slate-500 transition-all placeholder:text-slate-700">
            </div>
        </form>
    </div>
    ...
    <div id="summaryCards" class="grid grid-cols-2 md:grid-cols-5 gap-6">
        <div class="bg-white p-6 rounded-[2rem] border border-blue-50 shadow-sm flex flex-col gap-4">
            <div class="w-10 h-10 bg-blue-50 text-blue-500 rounded-full flex items-center justify-center font-bold">P</div>
            <div>
                <p class="text-[10px] font-bold text-slate-300 uppercase mb-1 text-center">รอดำเนินการ</p>
                <p class="text-3xl font-bold text-slate-800 leading-none text-center" data-summary="pending">{{ $pendingRequisitions }}</p>
            </div>
        </div>
        <div class="bg-white p-6 rounded-[2rem] border border-emerald-50 shadow-sm flex flex-col gap-4">
            <div class="w-10 h-10 bg-emerald-50 text-emerald-500 rounded-full flex items-center justify-center font-bold">F</div>
            <div>
                <p class="text-[10px] font-bold text-slate-300 uppercase mb-1 text-center">เสร็จสิ้น</p>
                <p class="text-3xl font-bold text-slate-800 leading-none text-center" data-summary="approved">{{ $approvedRequisitions }}</p>
            </div>
        </div>
        <div class="bg-white p-6 rounded-[2rem] border border-red-50 shadow-sm flex flex-col gap-4">
            <div class="w-10 h-10 bg-red-50 text-red-500 rounded-full flex items-center justify-center font-bold">C</div>
            <div>
                <p class="text-[10px] font-bold text-slate-300 uppercase mb-1 text-center">ยกเลิก</p>
                <p class="text-3xl font-bold text-slate-800 leading-none text-center" data-summary="cancelled">{{ $cancelledRequisitions }}</p>
            </div>
        </div>
        <div class="bg-white p-6 rounded-[2rem] border border-orange-50 shadow-sm flex flex-col gap-4">
            <div class="w-10 h-10 bg-orange-50 text-orange-500 rounded-full flex items-center justify-center font-bold">R</div>
            <div>
                <p class="text-[10px] font-bold text-slate-300 uppercase mb-1 text-center">ไม่อนุมัติ</p>
                <p class="text-3xl font-bold text-slate-800 leading-none text-center" data-summary="rejected">{{ $rejectedRequisitions ?? 0 }}</p>
            </div>
        </div>
        <div class="bg-slate-800 p-6 rounded-[2rem] shadow-xl shadow-slate-200 flex flex-col gap-4 text-white">
            <div class="w-10 h-10 bg-slate-700 text-slate-300 rounded-full flex items-center justify-center font-bold">T</div>
            <div>
                <p class="text-[10px] font-bold text-slate-500 uppercase mb-1 text-center">คำขอทั้งหมด</p>
                <p class="text-3xl font-bold text-white leading-none text-center underline decoration-red-600 decoration-4 underline-offset-8" data-summary="total">{{ $totalRequisitions }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col">
            <div class="flex items-start justify-between mb-8">
                <div>
                    <h2 class="text-sm font-bold text-slate-800 uppercase">ความคืบหน้ารายเดือน</h2>
                    <p class="text-[10px] text-slate-400 font-bold uppercase mt-1 leading-none">Status Breakdown per Month</p>
                </div>
                <div class="flex flex-col gap-2">
                    <label class="flex items-center gap-3 cursor-pointer group">
                        ...
                        <span class="text-[10px] font-bold text-slate-400 uppercase">Show Legend</span>
                    </label>
                    <label class="flex items-center gap-3 cursor-pointer group">
        ...
        <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col">
            <div class="mb-8 items-start">
                 <h2 class="text-sm font-bold text-slate-800 uppercase">พัสดุยอดนิยม (TOP 5)</h2>
                 <p class="text-[10px] text-slate-400 font-bold uppercase mt-1 leading-none">Top 5 Requisitioned Items</p>
            </div>
        ...
        <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col">
             <div class="mb-8">
                 <h2 class="text-sm font-bold text-slate-800 uppercase">แนวโน้มการเบิกสะสม</h2>
                 <p class="text-[10px] text-slate-400 font-bold uppercase mt-1 leading-none">Monthly Item Quantity Trend</p>
            </div>
            <div class="flex-1 min-h-[300px] flex items-center justify-center">
    ...
    <div class="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm">
        <div class="flex items-center justify-between mb-8">
            <div>
                <h2 class="text-sm font-bold text-slate-800 uppercase">สรุปงบประมาณรายจ่ายรายเดือน</h2>
                <p class="text-[10px] text-slate-400 font-bold uppercase mt-1 leading-none">Monthly Total Expenditure (Value in THB)</p>
            </div>
            <div class="flex items-center gap-6">
                <label class="flex items-center gap-3 cursor-pointer group">
                    <span class="text-[10px] font-bold text-slate-400 uppercase">Filter Results</span>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <span class="text-[10px] font-bold text-emerald-600 uppercase">Real-time Data Active</span>
                </div>
            </div>
        </div>
    ...
    <div id="loadingOverlay" class="hidden fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[200] flex items-center justify-center">
        <div class="bg-white p-10 rounded-[3rem] shadow-2xl flex flex-col items-center gap-6 animate-zoom-in">
            ...
            <div class="text-center">
                <p class="text-base font-bold text-slate-800 uppercase">กำลังประมวลผลข้อมูล</p>
                <p class="text-xs font-semibold text-slate-400 mt-1">Syncing with server analytics...</p>
            </div>
        </div>
    </div>
</div>
    ...
    function renderCharts(data){
        ...
        barChart=new Chart(document.getElementById('monthlyBarChart'),{
            ...
            options:{
                ...
                plugins:{
                    legend:{display:showLegend,position:'bottom',labels:{font:{weight:'700',size:11,family:'inherit'}}},
                    tooltip:{backgroundColor:'#1e293b',padding:12,titleFont:{weight:'700',size:13},bodyFont:{weight:'600'},callbacks:{label:(ctx)=>{const label=ctx.dataset.label||'';const val=ctx.parsed.y;return percentMode?`${label}: ${val.toFixed(1)}%`:`${label}: ${val} REQS`;}}}
                },
                scales:{
        ...
        document.getElementById('topItemsList').innerHTML=topItems.map((i,idx)=>`
            <li class='flex justify-between items-center bg-slate-50/50 p-2 px-4 rounded-xl border border-slate-50'>
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-[11px] font-bold text-slate-700 uppercase truncate max-w-[150px]">${i.name}</span>
                </div>
