		...
		<form action="{{ route('items.update', $item->item_id) }}" method="POST" enctype="multipart/form-data"
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
				...
				<div>
					...
					<input type="number" id="quantity" name="quantity" min="0"
						value="{{ old('quantity', $item->quantity) }}" class="input input-bordered w-full" required>
				</div>
				<div>
					...
					<input type="number" step="0.01" id="per_unit" name="per_unit"
						value="{{ old('per_unit', $item->per_unit) }}" class="input input-bordered w-full" required>
				</div>
				<div>
					...
					<input type="file" id="item_pic" name="item_pic" accept="image/*"
						class="file-input file-input-warning w-full">
					@if($item->item_pic)
						<div class="mt-2">
							<img src="{{ asset('images/items/' . $item->item_pic) }}" alt="Current Image"
								class="h-28 rounded border border-gray-300/60">
						</div>
					@endif
				</div>
			</div>
			<div>
				...
				<textarea id="description" name="description" rows="4" class="textarea textarea-bordered w-full"
					placeholder="รายละเอียดเพิ่มเติม">{{ old('description', $item->description) }}</textarea>
			</div>
			<div class="flex items-center space-x-4">
				...
				<a href="{{ route('items.index') }}" class="btn btn-error text-white">
