...
<div class="max-w-5xl mx-auto px-4 py-8 space-y-8 uppercase tracking-tight">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] shadow-sm border border-orange-50 animate-zoom-in">
        <div class="flex items-center gap-5">
            ...
            <div>
                <h1 class="text-2xl font-black text-slate-800 tracking-tighter italic leading-none">ใบเบิกพัสดุ (รอดำเนินการ)</h1>
                <p class="text-[13px] text-slate-400 font-bold mt-1.5 flex items-center gap-2">
                    ...
                    <span>•</span>
                    <span class="italic">กำลังรอฝ่ายพัสดุตรวจสอบ</span>
                </p>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="flex flex-col gap-9 pb-2">
                    <div>
                        ...
                        <p class="text-[11px] text-slate-400 font-bold mt-1 uppercase italic">{{ optional($requisition->request_date)->format('d M Y | H:i') }}</p>
                    </div>
                    <div>
                        <p class="text-[14px] font-black text-slate-400 leading-none">รอพิจารณา / ตรวจสอบพัสดุ</p>
                        <p class="text-[11px] text-orange-400 font-bold mt-1 uppercase italic">In Queue for Approval</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-slate-800 p-8 rounded-[2.5rem] shadow-xl shadow-slate-200 text-white flex flex-col justify-between">
            <div>
                ...
                <p class="text-[13px] font-bold text-slate-300 italic leading-relaxed">"ใบเบิกของคุณได้รับการจัดลำดับแล้ว กรุณารอการติดต่อกลับ หรือสถานะการอัปเดตจากเจ้าหน้าที่ HAMS"</p>
            </div>
            <div class="pt-6 border-t border-slate-700 mt-6">
                ...
                <p class="text-3xl font-black italic font-mono text-orange-400">฿{{ number_format($requisition->total_price, 2) }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
        <div class="p-8 border-b border-slate-50 flex items-center justify-between">
            <div class="flex items-center gap-3">
                ...
                <h2 class="text-lg font-black text-slate-800 tracking-tight italic uppercase">รายการพัสดุที่รอเบิก</h2>
            </div>
        ...
        <div class="p-6">
            <div class="grid grid-cols-1 gap-4">
                ...
                <div class="flex items-center justify-between p-5 bg-slate-50/50 rounded-3xl border border-slate-100 hover:border-orange-200 transition-colors group">
                    <div class="flex items-center gap-5">
                        <div class="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-slate-300 font-black italic border border-slate-100 shadow-sm group-hover:text-orange-400 transition-colors">
                            {{ str_pad($index + 1, 2, '0', STR_PAD_LEFT) }}
                        </div>
                        <div>
                            <p class="text-[15px] font-black text-slate-700">{{ $item->item->name ?? '-' }}</p>
                            <p class="text-[11px] font-bold text-slate-400 uppercase italic">Unit Price: ฿{{ number_format($item->item->per_unit ?? 0, 2) }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-8 text-right">
                        <div class="flex flex-col">
                            <span class="text-[9px] font-black text-slate-300 uppercase tracking-widest">Qty</span>
                            <span class="text-[16px] font-black text-slate-700 italic">{{ $item->quantity }} <span class="text-[10px] text-slate-400 font-normal">ชิ้น</span></span>
                        </div>
                        <div class="flex flex-col w-24">
                            <span class="text-[9px] font-black text-slate-300 uppercase tracking-widest">Total</span>
                            <span class="text-[16px] font-black text-orange-600 font-mono italic">฿{{ number_format(($item->item->per_unit ?? 0) * $item->quantity, 2) }}</span>
                        </div>
                    </div>
                </div>
                @endforeach
        ...
        <div class="p-8 bg-slate-50 border-t border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6">
            <div class="flex items-center gap-4 text-slate-400">
                ...
                <p class="text-[12px] font-medium leading-relaxed italic">คุณยังสามารถยกเลิกใบเบิกนี้ได้ก่อนที่เจ้าหน้าที่จะทำการเริ่มขั้นตอน <span class="text-orange-500 font-black italic">"กำลังดำเนินการ"</span></p>
            </div>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        document.querySelector('.btn-cancel-req').addEventListener('click', function (e) {
            ...
            Swal.fire({
                ...
                html: '<p class="text-sm text-slate-500 font-medium leading-relaxed italic">"ใบเบิกของคุณจะถูกระงับทันที และไม่สามารถเรียกกลับมาได้"</p>',
                icon: 'warning',
