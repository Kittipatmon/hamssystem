...
<div class="max-w-[1400px] mx-auto px-4 py-8 space-y-8 uppercase tracking-tight">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 animate-zoom-in">
        <div class="flex items-center gap-5">
            ...
            <div>
                <h1 class="text-2xl font-black text-slate-800 tracking-tighter leading-none">กำลังจัดเตรียมพัสดุ</h1>
                <p class="text-[13px] text-slate-400 font-bold mt-1.5 flex items-center gap-2">
                    ...
                    <span>•</span>
                    <span class="uppercase tracking-wide">โปรดตรวจสอบพัสดุให้ครบถ้วนก่อนบันทึก</span>
                </p>
            </div>
        </div>
        <div class="flex items-center gap-3 no-print">
            <a href="{{ route('requisitions.reqchecklist') }}" 
               class="px-6 py-3 bg-slate-50 hover:bg-slate-100 text-slate-500 font-black rounded-2xl border border-slate-100 transition-all active:scale-95 text-sm uppercase tracking-widest">
                <i class="fa-solid fa-arrow-left mr-2 opacity-50"></i> back to worklist
            </a>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4 animate-zoom-in" style="animation-delay: 0.1s">
        <div class="md:col-span-2 bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex items-center gap-5">
            <div class="w-14 h-14 bg-blue-50 text-blue-600 rounded-2xl flex items-center justify-center">
                <i class="fa-solid fa-user-tie text-xl"></i>
            </div>
            <div>
                <p class="text-[9px] font-black text-slate-300 uppercase tracking-widest leading-none mb-1.5">ผู้ขอเบิก (Requester)</p>
                <div class="flex flex-col">
                    <span class="text-[18px] font-black text-slate-700 leading-tight">คุณ{{ $requisition->user->fullname }}</span>
                    <span class="text-[10px] font-bold text-slate-400 mt-1 uppercase">{{ $requisition->user->department->department_name ?? "-" }} / {{ $requisition->user->section->section_code ?? "-" }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex items-center gap-5">
            ...
            <div>
                <p class="text-[9px] font-black text-slate-300 uppercase tracking-widest leading-none mb-1.5">ความคืบหน้า (Progress)</p>
                <div class="flex items-baseline gap-1">
                    <span id="progress-text" class="text-3xl font-black text-slate-800 font-mono">
                        {{ $requisition->requisition_items->where('check_item', 1)->count() }} / {{ $requisition->requisition_items->count() }}
                    </span>
                    <span class="text-[10px] font-black text-slate-300 uppercase">Items</span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 flex items-center gap-5">
            ...
            <div>
                <p class="text-[9px] font-black text-slate-300 uppercase tracking-widest leading-none mb-1.5">วันที่ขอ (Req. Date)</p>
                <p class="text-[16px] font-black text-slate-700 leading-none uppercase">{{ optional($requisition->request_date)->format('d / M / Y') }}</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden animate-zoom-in" style="animation-delay: 0.2s">
        <div class="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
            <div class="flex items-center gap-3">
                ...
                <h2 class="text-lg font-black text-slate-800 tracking-tight uppercase">รายการที่ต้องจัดสิ่ง (Checklist)</h2>
            </div>
            <div class="flex items-center gap-3">
                <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Verify all items to finalize</span>
            ...
            <table class="w-full text-left border-collapse min-w-[1000px]">
                <thead>
                    <tr class="bg-slate-100/30">
                        <th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest rounded-l-2xl text-center w-20">#</th>
                        <th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Item Specification / SKU</th>
                        <th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Required QTY</th>
                        <th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-right">Est. Value</th>
                        <th class="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center rounded-r-2xl w-48">Status / Verification</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50">
                    @foreach ($requisition->requisition_items as $index => $item)
                    <tr class="hover:bg-slate-50 transition-all duration-200 group {{ $item->check_item ? 'bg-emerald-50/20' : '' }}" id="row-{{ $item->requistionitem_id }}">
                        <td class="px-6 py-6 text-center text-slate-300 font-black font-mono">{{ str_pad($index + 1, 2, '0', STR_PAD_LEFT) }}</td>
                        <td class="px-6 py-6 font-black text-slate-700">
                             <div class="flex flex-col">
                                <span class="text-[15px] group-hover:text-red-600 transition-colors uppercase leading-tight">{{ $item->item->name ?? '-' }}</span>
                                <span class="text-[9px] text-slate-400 font-black uppercase mt-1.5 bg-slate-100 px-2 py-0.5 rounded border border-slate-200 w-fit group-hover:bg-white transition-colors">SKU: {{ $item->item->item_code ?? 'N/A' }}</span>
                             </div>
                        </td>
                        <td class="px-6 py-6 text-center">
                            <span class="inline-block px-5 py-2 bg-slate-900 text-white rounded-xl font-black text-[16px] shadow-lg shadow-slate-100">
                                {{ $item->quantity }}
                            </span>
                        </td>
                        <td class="px-6 py-6 text-right font-black text-slate-400 font-mono tracking-tight">
                        ...
                        <td class="px-6 py-6 text-center">
                            <div class="flex flex-col items-center gap-2">
                                <label class="inline-flex items-center cursor-pointer group/label relative">
                                    <input type="checkbox"
                                        class="sr-only check-item-checkbox peer"
                                        data-id="{{ $item->requistionitem_id }}"
                                        @if($item->check_item) checked @endif
                                    >
                                    <div class="w-12 h-12 bg-white border-2 border-slate-200 rounded-2xl flex items-center justify-center transition-all group-hover/label:border-red-400 group-hover/label:shadow-lg shadow-sm
                                                peer-checked:bg-emerald-500 peer-checked:border-emerald-500 peer-checked:shadow-emerald-100 peer-checked:scale-110">
                                        <i class="fa-solid fa-check text-slate-100 peer-checked:text-white text-xl transition-all {{ $item->check_item ? 'text-white' : '' }}"></i>
                                    </div>
                                </label>
                                <span class="check-indicator text-[8px] font-black uppercase tracking-widest {{ $item->check_item ? 'text-emerald-500' : 'text-slate-300' }}">
                                    {{ $item->check_item ? 'Verified' : 'Waiting' }}
                                </span>
                            </div>
                        </td>
                    </tr>
        ...
        <div class="p-8 bg-slate-900 text-white flex flex-col md:flex-row items-center justify-between gap-6">
            <div class="flex items-center gap-5">
                ...
                <div>
                    <p class="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1.5">สรุปมูลค่าประเมิน (Valuation Total)</p>
                    <p class="text-[14px] font-black text-slate-400 leading-none uppercase tracking-wide">Combined Estimated Value of this Pack</p>
                </div>
            </div>
            <div class="text-right">
                <span class="text-4xl font-black font-mono text-orange-400 decoration-slate-700 decoration-wavy decoration-2 underline-offset-[12px] underline">
        ...
        <!-- Footer Actions -->
        <div class="p-10 bg-slate-50/50 border-t border-slate-100 flex flex-col md:flex-row items-center justify-center gap-6 no-print">
            <button class="w-full md:w-96 h-20 bg-emerald-500 hover:bg-emerald-600 text-white font-black rounded-3xl shadow-2xl shadow-emerald-100 transition-all active:scale-95 flex items-center justify-center gap-4 btn-submit-req {{ $allChecked ? '' : 'hidden' }}"
                    data-id="{{ $requisition->requisitions_id }}">
                <div class="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center">
                    <i class="fa-solid fa-paper-plane text-xl"></i>
                </div>
                <div class="flex flex-col items-start leading-none gap-1">
                    <span class="text-[15px] uppercase tracking-tighter">จัดเตรียมเรียบร้อยแล้ว</span>
                    <span class="text-[10px] opacity-70 uppercase tracking-widest">finalize & notify user</span>
                </div>
            </button>
            <button class="w-full md:w-96 h-20 bg-white border-2 border-slate-200 text-slate-400 hover:border-red-400 hover:text-red-500 font-black rounded-3xl shadow-sm transition-all active:scale-95 flex items-center justify-center gap-4 btn-cancel-req group"
                    data-id="{{ $requisition->requisitions_id }}">
                <div class="w-10 h-10 bg-slate-50 group-hover:bg-red-50 rounded-xl flex items-center justify-center transition-colors">
                    <i class="fa-solid fa-triangle-exclamation text-xl"></i>
                </div>
                <div class="flex flex-col items-start leading-none gap-1">
                    <span class="text-[15px] uppercase tracking-tighter">พบปัญหา / ยกเลิกใบเบิก</span>
                    <span class="text-[10px] opacity-70 uppercase tracking-widest">report error or cancel</span>
                </div>
            </button>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        async function premiumPrompt({ title, text, confirmText, cancelText, placeholder, icon, confirmColor }) {
            return Swal.fire({
                title: `<span class="text-slate-800 font-black tracking-tighter uppercase">${title}</span>`,
                html: `<p class="text-[13px] text-slate-500 font-bold leading-relaxed uppercase">${text}</p>`,
                icon: icon || 'warning',
                input: 'textarea',
