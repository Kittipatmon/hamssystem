    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 space-y-10">
        ...
        <div class="header-frame reveal-item">
            <div class="flex items-center gap-5">
                ...
                <div class="space-y-1">
                    <h1 class="text-3xl font-black text-slate-800 tracking-tight leading-none uppercase md:normal-case">
                        ตะกร้าอุปกรณ์</h1>
                    <div class="flex items-center gap-2 flex-wrap">
                        <a href="{{ route('welcome') }}"
                            class="breadcrumb-item hover:text-red-600 transition-colors">หน้าหลัก</a>
                        <i class="fa-solid fa-chevron-right text-[8px] text-slate-300"></i>
                        <a href="{{ route('items.itemsalllist') }}"
                            class="breadcrumb-item hover:text-red-600 transition-colors">รายการอุปกรณ์</a>
                        <i class="fa-solid fa-chevron-right text-[8px] text-slate-300"></i>
        ...
        @if ($cart_items->count() == 0)
            <!-- 🛒 EMPTY STATE -->
            <div
                class="bg-white rounded-[3rem] p-20 shadow-xl border border-slate-50 flex flex-col items-center text-center reveal-item">
                ...
                <p class="text-slate-400 mb-10 max-w-sm leading-relaxed">
                    กรุณาเลือกอุปกรณ์ที่คุณต้องการเบิกใช้งานเข้าระบบก่อนทำรายการครับ</p>
        ...
        @else
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-10">
                        ...
                        <div class="item-card reveal-item" style="animation-delay: {{ $loop->index * 0.1 }}s">
                            ...
                            <div class="flex-1 min-w-0 pr-0 sm:pr-10">
                                <div class="flex justify-between items-start mb-2 group-hover:bg-slate-50">
                                    <div>
                                        <span
                                            class="text-[10px] font-black text-red-500 uppercase tracking-widest block">{{ optional($cart_item->item)->item_code }}</span>
                                        <h3 class="text-2xl font-black text-slate-800 tracking-tight leading-tight mt-1">
                                            {{ optional($cart_item->item)->name }}</h3>
                                    </div>
                                ...
                                <p class="text-sm text-slate-400 italic mb-6 line-clamp-1 opacity-80">
                                    {{ optional($cart_item->item)->description ?: 'ไม่มีรายละเอียดพัสดุอุปกรณ์' }}</p>
                                ...
                                <div class="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
                                        ...
                                        <form action="{{ route('cartitem.update', $cart_item->cart_id) }}" method="POST"
                                            ...
                                            <div class="qty-pill">
                                                ...
                                                <input type="text" name="display_quantity"
                                                    value="{{ $cart_item->cart_quantity ?? 1 }}" readonly class="qty-input" />
                                    ...
                                    <div class="text-left sm:text-right">
                                        <span class="text-[10px] font-black text-slate-300 uppercase tracking-widest block">Total
                                            unit cost</span>
                                        <div class="price-text">
                                            <span
                                                class="text-[0.6em] mr-1">฿</span>{{ number_format((optional($cart_item->item)->per_unit ?? 0) * $cart_item->cart_quantity, 2) }}
                                        </div>
                                        <span class="text-[11px] font-bold text-slate-300">@
                                            ฿{{ number_format(optional($cart_item->item)->per_unit, 2) }} each</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="sidebar-frame reveal-item" style="animation-delay: 0.3s">
                        ...
                        <div class="space-y-4 mb-10">
                            <div class="flex justify-between items-center px-2">
                                ...
                                <span
                                    class="text-sm font-black text-slate-800 bg-slate-50 px-4 py-1 rounded-full border border-slate-100">{{ $cart_items->sum('cart_quantity') }}
                                    ชิ้น</span>
                            </div>
                            <div class="flex justify-between items-center px-2">
                            ...
                            <div
                                class="py-8 bg-slate-900 border-4 border-white shadow-2xl rounded-[2.5rem] flex flex-col items-center justify-center mt-6">
                                <span
                                    class="text-[10px] font-black text-white/50 uppercase tracking-[0.4em] mb-2 leading-none">GRAND
                                    TOTAL</span>
                ...
                return ((optional($item->item)->per_unit ?? 0) * ($item->cart_quantity ?? 0)); }), 2) }}
                        ...
                        <form id="checkout-form" action="{{ route('cartitem.checkout') }}" method="POST" class="hidden">@csrf
                        </form>
                    </div>
                </div>
            </div>
        ...
        <div class="modal-box rounded-[3rem] p-0 overflow-hidden max-w-2xl bg-white">
            <div class="p-8 bg-slate-900 flex justify-between items-center text-white">
                ...
                <form method="dialog"><button
                        class="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center text-white hover:bg-white/20 transition-all">✕</button>
                </form>
            </div>
            <div class="p-10">
                <div
                    class="w-full h-80 rounded-[2rem] overflow-hidden bg-slate-50 mb-8 border border-slate-100 shadow-inner group">
                    <img id="modalImage" src=""
                        class="w-full h-full object-contain group-hover:scale-105 transition-transform duration-1000" />
                </div>
                <div id="modalDescription"
                    class="text-slate-600 leading-relaxed text-sm bg-slate-50 p-8 rounded-[2rem] border border-slate-200 italic font-medium">
                </div>
            </div>
            <div class="px-10 pb-10">
                <form method="dialog">
                    <button
                        class="w-full py-4 bg-slate-900 text-white font-black rounded-2xl hover:bg-black transition-all shadow-xl active:scale-95 text-xs uppercase tracking-widest">
                        ตกลง
                    </button>
                </form>
            </div>
