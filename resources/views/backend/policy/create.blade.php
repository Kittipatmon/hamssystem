...
<div class="min-h-screen bg-slate-100 dark:bg-zinc-950 px-4 sm:px-6 lg:px-8 py-8">
    <div class="max-w-4xl mx-auto space-y-6 font-noto">
        ...
        <div class="flex items-center justify-between">
            <a href="{{ route('backend.policy.index', ['type' => request()->type ?? 'policy']) }}" 
        ...
        <div class="bg-white dark:bg-zinc-900 border border-slate-300 dark:border-zinc-800 shadow-sm overflow-hidden">
            <div class="bg-slate-50 dark:bg-zinc-800 border-b border-slate-300 dark:border-zinc-700 px-6 py-4">
                <h2 class="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                    <i class="fa-solid fa-file-circle-plus text-red-600"></i>
                    <span>เพิ่มข้อมูล{{ request()->type === 'operation' ? 'ขั้นตอนการดำเนินงาน' : 'นโยบาย' }}</span>
                </h2>
            </div>

            <form action="{{ route('backend.policy.store') }}" method="POST" class="p-6 sm:p-8 space-y-6">
                @csrf
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="content" rows="6"
                        ...
                        class="w-full px-4 py-3 rounded border border-slate-300 dark:border-zinc-700 bg-slate-50 dark:bg-zinc-800 focus:bg-white focus:ring-1 focus:ring-red-600 focus:border-red-600 outline-none transition-all text-sm text-slate-900 dark:text-white font-medium"></textarea>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-2">
                        ...
                        <select name="type" required
                            ...
                            <option value="operation" {{ request()->type === 'operation' ? 'selected' : '' }}>ขั้นตอนการทำงาน (Operation)</option>
                        </select>
                    </div>
                    <div class="space-y-2">
