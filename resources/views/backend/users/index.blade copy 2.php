...
@section('content')
    ...
    <div class="space-y-6">
        {{-- Header Section --}}
        <div
            class="flex flex-col md:flex-row justify-between items-center gap-6 pb-6 border-b border-gray-200 dark:border-white/10">
            <div>
                <h1 class="text-3xl font-bold text-kumwell-red flex items-center gap-3">
                    <i class="fa-solid fa-users-gear text-2xl opacity-80"></i>
                    จัดการข้อมูลพนักงาน
                </h1>
                <p class="text-gray-500 dark:text-gray-400 mt-1 text-sm">จัดการข้อมูลพื้นฐาน, ระดับพนักงาน และสถานะการทำงาน
                </p>
            </div>
            <div class="flex flex-wrap gap-3 w-full md:w-auto">
                <button type="button" id="toggle-filter"
                    class="btn btn-ghost border-gray-300 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-white/5 flex-grow md:flex-none">
        ...
        <form id="filter-form" method="GET" action="{{ route('users.index') }}"
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end mb-4">
                <div class="form-control">
                    ...
                    <input type="text" name="emp_code" placeholder="ค้นหารหัส..." value="{{ request('emp_code') }}"
                ...
                <div class="form-control">
                    ...
                    <select name="employee_type" class="select select-bordered select-sm w-full dark:bg-gray-700">
                        ...
                        <option value="รายเดือน" {{ request('employee_type') === 'รายเดือน' ? 'selected' : '' }}>รายเดือน
                        </option>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                ...
                @endphp
                <select name="role" class="select select-bordered select-sm w-full dark:bg-gray-700">
                    <option value="">ระดับสิทธิ์ (ทั้งหมด)</option>
                ...
                <table class="table table-md w-full">
                    <thead class="kumwell-table-header dark:text-gray-300">
                        <tr class="border-b border-gray-200 dark:border-white/5">
                            ...
                            <th class="text-center">สิทธิ์รายงาน</th>
                    ...
                    <tbody id="users-body"
                        ...
                        @forelse($users as $user)
                            <tr
                                class="hover:bg-gray-50 dark:hover:bg-white/5 transition-all duration-200 border-b border-gray-100 dark:border-white/5">
                                <td class="pl-6">
                                    <span
                                        class="font-mono text-sm bg-gray-100 dark:bg-white/10 px-2 py-1 rounded text-kumwell-red font-bold">
                                        {{ $user->emp_code }}
                                    </span>
                                </td>
                                <td>
                                ...
                                <td class="text-center">
                                    <div
                                        class="kumwell-badge bg-{{ $user->level_user_color }}/10 text-{{ $user->level_user_color }} border border-{{ $user->level_user_color }}/20">
                                ...
                                <td class="text-center">
                                    ...
                                    <div
                                        class="kumwell-badge bg-{{ $user->hams_status_color }}/10 text-{{ $user->hams_status_color }} border border-{{ $user->hams_status_color }}/20">
                                 ...
                                 <td class="text-center">
                                    ...
                                    <button type="button" onclick="toggleHamsEditor({{ $user->id }}, this)"
                                        class="btn btn-xs rounded-full whitespace-nowrap px-3 border-none transition-all duration-300 hover:shadow-md {{ $isEditor ? 'bg-emerald-500 text-white hover:bg-emerald-600' : 'bg-slate-100 text-slate-600 hover:bg-slate-200' }}">
                                        <div class="flex items-center gap-1.5">
                                            <i
                                                class="fa-solid {{ $isEditor ? 'fa-check-circle' : 'fa-circle-plus opacity-50' }} text-[10px]"></i>
                                            <span class="text-[11px] font-bold">HAMS Editor</span>
                                        </div>
                                    </button>
                                    @if ($isEditor && $user->hamsPermissionLatestLog?->grantedBy)
                                        <div class="text-[9px] text-emerald-600 mt-1 font-bold opacity-80 grantor-info">
                                            <i class="fa-solid fa-user-check mr-1"></i>โดย:
                                            {{ $user->hamsPermissionLatestLog->grantedBy->fullname }}
                                        </div>
                                    @endif
                                </td>
                                <td class="text-center">
                                    <div
                                        class="kumwell-badge bg-{{ $user->status_color }}/10 text-{{ $user->status_color }} border border-{{ $user->status_color }}/20">
                                        {!! $user->status_icon !!}
                                        {{ $user->status_label }}
                                    </div>
                                </td>
                                <td class="pr-6">
                                    <div class="flex justify-center gap-2">
                                        <a href="{{ route('users.show', $user->id) }}"
                                            class="btn btn-circle btn-ghost btn-xs hover:bg-info/20 hover:text-info transition-all"
                                            title="ดูข้อมูล">
                                            <i class="fa-solid fa-eye"></i>
                                        </a>
                                        <a href="{{ route('users.edit', $user->id) }}"
                                            class="btn btn-circle btn-ghost btn-xs hover:bg-warning/20 hover:text-warning transition-all"
                                            title="แก้ไข">
                                        ...
                                        <form action="{{ route('users.destroy', $user->id) }}" method="POST"
                                            ...
                                            @method('DELETE')
                                            <button type="submit"
                                                class="btn btn-circle btn-ghost btn-xs hover:bg-error/20 hover:text-error transition-all"
                                                title="ลบ">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function renderRows(items) {
                ...
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors border-b border-gray-100 dark:border-white/5">
                    ...
                    <td>
                        <div class="flex flex-col gap-1">
                            <span class="text-xs font-medium text-gray-700 dark:text-gray-300">
                                <i class="fa-solid fa-building-user mr-1 opacity-50"></i> ${safe(u.department?.name)}
                            </span>
                            ${(safe(u.division?.division_name) !== '-' || safe(u.section?.section_code) !== '-') ? `
                                <span class="text-[10px] text-gray-400">
                                    ${safe(u.division?.division_name)} / ${safe(u.section?.section_code)}
                                </span>
                            ` : ''}
                    ...
                    <td class="text-center">
                        <button type="button" 
                            onclick="toggleHamsEditor(${u.id}, this)"
                            class="btn btn-xs rounded-full whitespace-nowrap px-3 border-none transition-all duration-300 hover:shadow-md ${u.hams_permission?.is_hams_editor ? 'bg-emerald-500 text-white hover:bg-emerald-600' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}">
                            <div class="flex items-center gap-1.5">
                                <i class="fa-solid ${u.hams_permission?.is_hams_editor ? 'fa-check-circle' : 'fa-circle-plus opacity-50'} text-[10px]"></i>
                                <span class="text-[11px] font-bold">HAMS Editor</span>
                            </div>
                        </button>
                        ${(u.hams_permission?.is_hams_editor && u.hams_permission_latest_log?.granted_by) ? `
                            <div class="text-[9px] text-emerald-600 mt-1 font-bold opacity-80 grantor-info">
                                <i class="fa-solid fa-user-check mr-1"></i>โดย: ${safe(u.hams_permission_latest_log.granted_by.fullname)}
                            </div>
                        ` : ''}
                    </td>
                    <td class="text-center">
            ...
            function renderPagination(pag) {
                ...
                let html =
                    `<div class="join shadow-sm border border-gray-200 dark:border-white/10 rounded-xl overflow-hidden">
                <a class="join-item btn btn-sm bg-white dark:bg-zinc-800 text-gray-500 hover:bg-gray-50 dark:hover:bg-white/5 border-r border-gray-200 dark:border-white/10 ${!prev ? 'btn-disabled opacity-50' : ''}" ${prev ? `href="${prev}"` : ''}>
                    <i class="fa-solid fa-chevron-left text-[10px]"></i>
                </a>`;

                const pages = calcWindow(cur, last);
                pages.forEach(p => {
                    if (p === '...') html += `<button class="join-item btn btn-sm btn-disabled bg-white dark:bg-zinc-800 text-gray-400 border-r border-gray-200 dark:border-white/10">...</button>`;
                    else {
                        const url = new URL(pag.path + '?page=' + p, window.location.origin);
                        new URLSearchParams(window.location.search).forEach((v, k) => {
                            if (k !== 'page') url.searchParams.set(k, v);
                        });
                        const activeClass = p === cur ? 'bg-kumwell-red text-white hover:bg-red-700' : 'bg-white dark:bg-zinc-800 text-gray-600 hover:bg-gray-50 dark:hover:bg-white/5';
                        html +=
                            `<a class="join-item btn btn-sm border-r border-gray-200 dark:border-white/10 ${activeClass}" href="${url}">${p}</a>`;
                    }
                });
                html +=
                    `<a class="join-item btn btn-sm bg-white dark:bg-zinc-800 text-gray-500 hover:bg-gray-50 dark:hover:bg-white/5 ${!next ? 'btn-disabled opacity-50' : ''}" ${next ? `href="${next}"` : ''}>
                ...
                try {
                    ...
                    if (data.success) {
                        if (data.is_hams_editor) {
                            btn.className = 'btn btn-xs rounded-full whitespace-nowrap px-3 border-none bg-emerald-500 text-white transition-all duration-300 hover:shadow-md hover:bg-emerald-600';
                            btn.innerHTML = '<div class="flex items-center gap-1.5"><i class="fa-solid fa-check-circle text-[10px]"></i><span class="text-[11px] font-bold">HAMS Editor</span></div>';
                            
                            // Update/Add grantor info
                            ...
                            btn.className = 'btn btn-xs rounded-full whitespace-nowrap px-3 border-none bg-slate-100 text-slate-600 transition-all duration-300 hover:shadow-md hover:bg-slate-200';
                            btn.innerHTML = '<div class="flex items-center gap-1.5"><i class="fa-solid fa-circle-plus opacity-50 text-[10px]"></i><span class="text-[11px] font-bold">HAMS Editor</span></div>';
