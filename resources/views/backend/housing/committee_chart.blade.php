...
@section('content')
    <div class="max-w-7xl mx-auto">
        ...
        {{-- Header --}}
        <div
            class="flex flex-col md:flex-row md:items-center justify-between mb-10 bg-white p-8 rounded-3xl shadow-sm border border-slate-100">
            <div>
            ...
            @if($isHams)
                <div class="flex gap-3">
                    <button onclick="document.getElementById('modal_add').showModal()"
                        class="mt-4 md:mt-0 bg-red-600 hover:bg-red-700 text-white px-6 py-3 rounded-2xl text-sm font-bold shadow-lg shadow-red-200 transition-all flex items-center justify-center gap-2">
            ...
            <div class="tree">
                                ...
                                <div class="level-container">
                                    ...
                                    @if($loop->first)
                                        <div class="role-header flex items-center gap-2">
                                            <span
                                                class="px-4 py-1.5 bg-slate-900 text-white text-[9px] font-black uppercase tracking-[2px] rounded-full shadow-lg border border-slate-700">
                                                {{ $role }}
                                            </span>
                                            @if($isHams)
                                                <button onclick="openAddModal('{{ $role }}')"
                                                    class="level-add-btn w-8 h-8 rounded-xl bg-emerald-500 text-white shadow-lg hover:rotate-90 flex items-center justify-center transition-all"
                                                    title="เพิ่มในสายนี้">
                                    ...
                                    <div
                                        class="node-card border-t-4 {{ $loop->parent->first ? 'border-red-600' : 'border-amber-500' }}">
                                        @if($user)
                                            <img src="{{ $user->photo_user ? asset($user->photo_user) : 'https://ui-avatars.com/api/?name=' . urlencode($user->fullname) . '&background=' . ($loop->parent->first ? 'dc2626' : 'f59e0b') . '&color=fff' }}"
                                                class="node-avatar" alt="Avatar">
                                            <h3 class="node-name">{{ $user->fullname }}</h3>
                                            <p class="node-dept">{{ optional($user->department)->name ?? '-' }}</p>
                                        @else
                                            <div class="node-avatar bg-gray-200 flex items-center justify-center rounded-full mx-auto mb-3">
                                                <i class="fa-solid fa-user-slash text-gray-400"></i>
                                            </div>
                                            <h3 class="node-name">ไม่พบข้อมูลพนักงาน</h3>
                                            <p class="node-dept text-xs opacity-50">ID: {{ $m->user_id }}</p>
                                        @endif

                                        @if($isHams)
                                            <div class="node-actions">
                                                <button onclick='editNode(@json($m))'
                                                    class="w-8 h-8 rounded-full bg-blue-50 text-blue-600 hover:bg-blue-100 flex items-center justify-center">
                                                ...
                                                <form action="{{ route('housing.committee.destroy', $m->id) }}" method="POST"
                                                    ...
                                                    @csrf @method('DELETE')
                                                    <button type="submit"
                                                        class="w-8 h-8 rounded-full bg-red-50 text-red-600 hover:bg-red-100 flex items-center justify-center">
                ...
                @else
                    <div class="py-20 text-center w-full">
                        <div
                            class="w-24 h-24 bg-red-50 text-red-600 rounded-full flex items-center justify-center mx-auto mb-6">
        ...
        <div class="modal-box bg-white rounded-3xl p-8 max-w-md">
            ...
            <form action="{{ route('housing.committee.store') }}" method="POST">
                ...
                <div class="space-y-5">
                    <div>
                        <label
                            class="text-xs font-bold text-slate-500 mb-2 block uppercase tracking-wider">เลือกพนักงาน</label>
                    ...
                    <div>
                        <label class="text-xs font-bold text-slate-500 mb-2 block uppercase tracking-wider">ตำแหน่ง /
                            สายงาน</label>
                        <select id="role_select" name="role" class="select select-bordered w-full rounded-2xl"
                        ...
                        <input type="text" id="role_input" placeholder="ระบุตำแหน่งใหม่"
                            class="input input-bordered w-full mt-2 hidden rounded-2xl">
                    </div>
                    <div>
                        <label class="text-xs font-bold text-slate-500 mb-2 block uppercase tracking-wider">ลำดับขั้น
                            (1=สูงสุด)</label>
                ...
                <div class="modal-action flex gap-3 mt-8">
                    <button type="button" onclick="modal_add.close()"
                        class="flex-1 font-bold text-slate-400">ยกเลิก</button>
                    <button type="submit"
                        class="flex-1 bg-red-600 text-white font-bold py-3 rounded-2xl shadow-lg shadow-red-100">บันทึก</button>
                </div>
            </form>
        </div>
            ...
            <form id="edit_form" method="POST">
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <select name="user_id" id="edit_user_id" class="select select-bordered w-full rounded-2xl select2-user-edit" required>
                            @foreach($users as $u)
                                @if($u->role === 'admin' || $u->emp_code === 'SYS-001') @continue @endif
                                <option value="{{ $u->id }}">{{ $u->fullname }}</option>
                            @endforeach
                    ...
                    <div>
                        ...
                        <select id="edit_role_select" name="role" class="select select-bordered w-full rounded-2xl"
