    ...
    <div class="mb-6">
        ...
        <div class="bg-white rounded-xl border border-slate-200 p-5 flex items-center gap-4">
            <div class="w-12 h-12 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-center shrink-0">
                <i class="fa-solid fa-file-signature text-slate-600 text-xl"></i>
            </div>
            <div>
                <h2 class="text-lg font-bold text-slate-800">{{ isset($item) ? 'แก้ไขแบบฟอร์มข้อตกลงการเข้าพักอาศัย' : 'แบบฟอร์มข้อตกลงการเข้าพักอาศัย' }}</h2>
                <p class="text-xs text-slate-400 mt-0.5">QF-HAMS-03 Rev.00 • กรุณากรอกข้อมูลให้ครบทุกช่องที่มีเครื่องหมาย <span class="text-red-500">*</span></p>
            </div>
        </div>
    </div>
    ...
    <form action="{{ isset($item) ? route('housing.agreement.update', $item->agreement_id) : route('housing.agreement.store') }}" method="POST">
        ...
        <div class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
            ...
            <div class="px-5 py-3 bg-slate-50 border-b border-slate-200">
                <h3 class="font-bold text-slate-700 text-xs uppercase tracking-wider flex items-center gap-2"><i class="fa-solid fa-user-tie text-red-500"></i> ข้อมูลพนักงาน</h3>
            </div>
            <div class="p-5 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-650 mb-1.5">คำนำหน้า <span class="text-red-500">*</span></label>
                        <select name="title" class="w-full rounded-lg border border-slate-300 text-xs h-10 px-3 focus:ring-1 focus:ring-red-500 focus:border-red-500" required>
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-xs font-bold text-slate-650 mb-1.5">ชื่อ-นามสกุล <span class="text-red-500">*</span></label>
                        <input type="text" name="full_name" value="{{ old('full_name', $item->full_name ?? ($userRequest ? ($userRequest->first_name . ' ' . $userRequest->last_name) : ((Auth::user()->first_name ?? '') . ' ' . (Auth::user()->last_name ?? '')))) }}"
                            class="w-full rounded-lg border border-slate-300 text-xs h-10 px-3 focus:ring-1 focus:ring-red-500 focus:border-red-500" required>
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-xs font-bold text-slate-650 mb-1.5">ตำแหน่ง <span class="text-red-500">*</span></label>
                        <input type="text" name="position" value="{{ old('position', $item->position ?? ($userRequest->position ?? ($user->position ?? ''))) }}" class="w-full rounded-lg border border-slate-300 text-xs h-10 px-3 focus:ring-1 focus:ring-red-500 focus:border-red-500" required>
                    </div>
                    <div>
                        <label class="block text-xs font-bold text-slate-650 mb-1.5">แผนก <span class="text-red-500">*</span></label>
                        <input type="text" name="department" value="{{ old('department', $item->department ?? ($userRequest->department ?? ($user->department->department_name ?? ''))) }}" class="w-full rounded-lg border border-slate-300 text-xs h-10 px-3 focus:ring-1 focus:ring-red-500 focus:border-red-500" required>
                    </div>
                    <div>
            ...
            <div class="px-5 py-3 bg-slate-50 border-t border-b border-slate-200">
                <h3 class="font-bold text-slate-700 text-xs uppercase tracking-wider flex items-center gap-2"><i class="fa-solid fa-house text-red-500"></i> ข้อมูลบ้านพัก</h3>
            </div>
            <div class="p-5 grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-xs font-bold text-slate-650 mb-1.5">ห้อง / ที่อยู่บ้านพัก</label>
                ...
                <div>
                    <label class="block text-xs font-bold text-slate-650 mb-1.5">จำนวนผู้อาศัย</label>
            ...
            {{-- ข้อตกลงและเงื่อนไข --}}
            <div class="px-5 py-3 bg-slate-50 border-t border-b border-slate-200">
                <h3 class="font-bold text-slate-700 text-xs uppercase tracking-wider flex items-center gap-2"><i class="fa-solid fa-scale-balanced text-red-500"></i> ข้อตกลงและเงื่อนไข</h3>
            </div>
            <div class="p-5">
                <div class="bg-slate-50 rounded-lg p-4 border border-slate-200 text-slate-700 space-y-2 mb-4">
                    <ol class="list-decimal pl-5 space-y-2 text-xs leading-relaxed">
                ...
                <div class="p-4 bg-red-50 rounded-lg border border-red-200">
                    <label class="flex items-center gap-2.5 cursor-pointer">
                        <input type="checkbox" id="accept_terms" required class="w-4 h-4 text-red-600 border-slate-350 rounded focus:ring-red-500" onchange="toggleSubmit(this.checked)">
                        <span class="text-xs text-red-800 font-bold">ข้าพเจ้ายอมรับข้อตกลงทั้งหมดข้างต้น <span class="text-red-500">*</span></span>
                    </label>
                </div>
            </div>
            ...
            <div class="px-5 py-4 bg-slate-50 border-t border-slate-200 flex justify-between items-center">
                ...
                <button type="submit" id="submit_btn" disabled class="px-8 py-2.5 bg-slate-800 text-white rounded-lg text-xs font-bold opacity-50 cursor-not-allowed shadow-sm transition-all flex items-center gap-1.5">
...
<script>
    function toggleSubmit(checked) {
        ...
        if (checked) {
            ...
            btn.classList.add('hover:bg-slate-900');
        } else {
