...
@section('content')
    <div class="max-w-4xl mx-auto">
        ...
        <div class="text-sm breadcrumbs text-slate-500 mb-4 px-2">
            <ul>
                <li><a href="{{ route('backend.bookingmeeting.rooms.index') }}" class="hover:text-teal-600"><i
                            class="fa-solid fa-door-open mr-2"></i> จัดการข้อมูลห้องประชุม</a></li>
                <li class="text-slate-700 font-medium">เพิ่มห้องประชุมใหม่</li>
            </ul>
        </div>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            ...
            <div class="bg-slate-50 border-b border-slate-200 p-6 flex items-center gap-3">
                <div
                    class="w-10 h-10 rounded-full bg-teal-100 text-teal-600 flex items-center justify-center text-lg shadow-inner">
                    <i class="fa-solid fa-plus"></i>
                </div>
                <div>
                    <h2 class="text-lg font-bold text-slate-800">ข้อมูลห้องประชุมใหม่</h2>
                    <p class="text-xs text-slate-500 mt-0.5">กรอกรายละเอียดเพื่อเปิดใช้งานห้องประชุมในระบบจอง</p>
                </div>
            </div>
            ...
            <form action="{{ route('backend.bookingmeeting.rooms.store') }}" method="POST" enctype="multipart/form-data"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <input type="text" name="room_name" value="{{ old('room_name') }}" required
                            ...
                            class="w-full h-11 px-4 rounded-xl border border-slate-300 focus:border-teal-500 focus:ring-2 focus:ring-teal-200 transition-all text-sm @error('room_name') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="capacity" value="{{ old('capacity') }}" required min="1"
                                ...
                                class="w-full h-11 pl-10 pr-4 rounded-xl border border-slate-300 focus:border-teal-500 focus:ring-2 focus:ring-teal-200 transition-all text-sm @error('capacity') border-red-500 @enderror">
                        </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="location" value="{{ old('location') }}"
                                ...
                                class="w-full h-11 pl-10 pr-4 rounded-xl border border-slate-300 focus:border-teal-500 focus:ring-2 focus:ring-teal-200 transition-all text-sm">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="floor" value="{{ old('floor') }}" placeholder="เช่น ชั้น 2"
                                class="w-full h-11 pl-10 pr-4 rounded-xl border border-slate-300 focus:border-teal-500 focus:ring-2 focus:ring-teal-200 transition-all text-sm">
                        </div>
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-slate-700 mb-2">สถานะการใช้งาน <span
                                class="text-red-500">*</span></label>
                        <select name="status"
                            class="w-full h-11 px-4 rounded-xl border border-slate-300 focus:border-teal-500 focus:ring-2 focus:ring-teal-200 transition-all text-sm">
                            <option value="1" {{ old('status', 1) == 1 ? 'selected' : '' }}>เปิดใช้งาน (ให้จองได้)</option>
                            <option value="0" {{ old('status') == '0' ? 'selected' : '' }}>ปิดใช้งานชั่วคราว</option>
                        </select>
                    </div>
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <div class="flex flex-wrap gap-4">
                            <label
                                ...
                                <input type="checkbox" name="has_projector" value="1" {{ old('has_projector') ? 'checked' : '' }} class="w-5 h-5 text-teal-600 rounded border-gray-300 focus:ring-teal-500">
                            ...
                            <label
                                class="flex items-center gap-3 p-3 border border-slate-200 rounded-xl cursor-pointer hover:bg-slate-50 transition-colors">
                                <input type="checkbox" name="has_video_conf" value="1" {{ old('has_video_conf') ? 'checked' : '' }} class="w-5 h-5 text-teal-600 rounded border-gray-300 focus:ring-teal-500">
                                <div class="flex items-center gap-2">
                                    <div
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <textarea name="description" rows="3" placeholder="ข้อมูลอื่นๆ ของห้องประชุม เช่น กฎการใช้งาน ฯลฯ"
                            class="w-full p-4 rounded-xl border border-slate-300 focus:border-teal-500 focus:ring-2 focus:ring-teal-200 transition-all text-sm">{{ old('description') }}</textarea>
                    </div>

                    <!-- Upload Images -->
                    <div class="col-span-1 md:col-span-2">
                        <label class="block text-sm font-semibold text-slate-700 mb-2">รูปภาพห้องประชุม <span
                                class="text-xs text-slate-400 font-normal">(สามารถเลือกได้หลายรูป)</span></label>

                        <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-xl hover:border-teal-400 hover:bg-teal-50/50 transition-colors cursor-pointer"
                            ...
                            <div class="space-y-1 text-center">
                                ...
                                <div class="flex items-center justify-center text-sm text-slate-600 mt-2">
                                    <span
                                        class="relative rounded-md font-medium text-teal-600 hover:text-teal-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-teal-500">
                ...
                <div class="pt-6 border-t border-slate-200 flex items-center justify-end gap-3">
                    ...
                    <button type="submit"
                        class="px-6 py-2.5 rounded-xl bg-teal-600 text-white font-medium hover:bg-teal-700 shadow-md shadow-teal-200 transition-colors flex items-center gap-2">
