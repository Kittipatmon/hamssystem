...
@section('title', 'เพิ่มห้องประชุมใหม่')
...
@section('content')
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="text-xs breadcrumbs text-slate-500 px-2">
            <ul>
                <li><a href="{{ route('backend.bookingmeeting.rooms.index') }}" class="hover:text-teal-600 font-semibold"><i
                            class="fa-solid fa-door-open mr-2"></i> จัดการข้อมูลห้องประชุม</a></li>
                <li class="text-slate-800 font-bold">เพิ่มห้องประชุมใหม่</li>
            </ul>
        </div>
        ...
        <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div class="bg-slate-50 border-b border-slate-200 p-5 flex items-center gap-3">
                <div class="w-10 h-10 rounded-lg bg-teal-50 text-teal-600 flex items-center justify-center text-lg shadow-inner">
                    <i class="fa-solid fa-plus"></i>
                </div>
                <div>
                    <h2 class="text-sm font-black text-slate-800 uppercase tracking-wider">บันทึกข้อมูลห้องประชุมใหม่</h2>
                    <p class="text-[11px] text-slate-400 font-semibold mt-0.5">กรอกรายละเอียดคุณลักษณะห้องประชุมเพื่อเปิดใช้สิทธิ์การจองระบบ</p>
                </div>
            </div>

            <form action="{{ route('backend.bookingmeeting.rooms.store') }}" method="POST" enctype="multipart/form-data"
                class="p-6 space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <input type="text" name="room_name" value="{{ old('room_name') }}" required
                            ...
                            class="w-full h-9 px-3 rounded border border-slate-300 bg-white focus:border-teal-500 focus:ring-1 focus:ring-teal-500 outline-none transition-all font-semibold text-slate-800">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="number" name="capacity" value="{{ old('capacity') }}" required min="1"
                                placeholder="เช่น 15"
                                class="w-full h-9 pl-9 pr-3 rounded border border-slate-300 bg-white focus:border-teal-500 focus:ring-1 focus:ring-teal-500 outline-none transition-all font-semibold text-slate-800">
                        </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input type="text" name="floor" value="{{ old('floor') }}" placeholder="เช่น ชั้น 3"
                                class="w-full h-9 pl-9 pr-3 rounded border border-slate-300 bg-white focus:border-teal-500 focus:ring-1 focus:ring-teal-500 outline-none transition-all font-semibold text-slate-800">
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status"
                            class="w-full h-9 px-3 bg-white border border-slate-300 rounded focus:border-teal-500 focus:ring-1 focus:ring-teal-500 outline-none transition-all font-bold text-slate-800">
                            <option value="1" {{ old('status', 1) == 1 ? 'selected' : '' }}>เปิดใช้งาน (ให้บุคลากรจองใช้งานได้)</option>
                            <option value="0" {{ old('status') == '0' ? 'selected' : '' }}>ปิดการใช้งานชั่วคราว / ปรับปรุง</option>
                        </select>
                    </div>
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <div class="flex flex-wrap gap-3 mt-1 font-semibold">
                            <label class="flex items-center gap-2 p-2.5 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 transition-colors">
                                <input type="checkbox" name="has_projector" value="1" {{ old('has_projector') ? 'checked' : '' }} class="w-4 h-4 text-teal-600 rounded border-slate-300 focus:ring-teal-500">
                            ...
                            <label class="flex items-center gap-2 p-2.5 border border-slate-200 rounded-lg cursor-pointer hover:bg-slate-50 transition-colors">
                                <input type="checkbox" name="has_video_conf" value="1" {{ old('has_video_conf') ? 'checked' : '' }} class="w-4 h-4 text-teal-600 rounded border-slate-300 focus:ring-teal-500">
                                <div class="flex items-center gap-1.5">
                    ...
                    <div class="col-span-1 md:col-span-2">
                        ...
                        <textarea name="description" rows="3" placeholder="ระบุกฎการจอง เงื่อนไขการใช้อุปกรณ์ หรือข้อความแนะนำอื่นของห้องประชุม..."
                            class="w-full p-3 rounded border border-slate-300 bg-white focus:border-teal-500 focus:ring-1 focus:ring-teal-500 outline-none transition-all font-semibold text-slate-800">{{ old('description') }}</textarea>
                    </div>

                    <!-- Upload Images -->
                    <div class="col-span-1 md:col-span-2">
                        <label class="block font-bold text-slate-700 mb-1.5">รูปภาพห้องประชุมประกอบ (อัปโหลดได้หลายรูปพร้อมกัน)</label>

                        <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-slate-300 border-dashed rounded-lg hover:border-teal-400 hover:bg-teal-50/20 transition-colors cursor-pointer"
                            ...
                            <div class="space-y-1 text-center font-bold text-slate-500">
                                ...
                                <div class="flex items-center justify-center text-[11px] mt-2">
                                    <span class="relative rounded font-bold text-teal-600 hover:text-teal-500">
                                        <span>อัปโหลดคลิกที่นี่</span>
                                        <input id="file-upload" name="image_file[]" type="file" class="sr-only" multiple
                                            accept="image/*" onchange="previewFiles()">
                                    </span>
                                    <p class="pl-1">หรือลากไฟล์ภาพมาจัดวาง</p>
                                </div>
                ...
                <div class="pt-5 border-t border-slate-200 flex items-center justify-end gap-3 font-semibold text-xs">
                    ...
                    <button type="submit"
                        class="px-5 py-2 rounded bg-teal-600 hover:bg-teal-700 text-white shadow transition-colors flex items-center gap-1.5">
                        <i class="fa-solid fa-save"></i> บันทึกข้อมูลห้อง
                    </button>
                </div>
            </form>
        </div>
