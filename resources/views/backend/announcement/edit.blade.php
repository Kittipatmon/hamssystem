...
@section('content')
    <div class="max-w-4xl mx-auto px-4 py-12">
        <!-- Breadcrumb & Header -->
        <div
            class="mb-10 text-center sm:text-left flex flex-col sm:flex-row justify-between items-center bg-white dark:bg-zinc-950 p-6 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-sm gap-4 transition-all">
            <div>
                <h1
                    class="text-3xl font-black text-zinc-900 dark:text-white flex items-center justify-center sm:justify-start gap-4">
                ...
                <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-2 font-medium">
                    แก้ไขข้อมูลประกาศด้านล่างเพื่ออัปเดตข้อมูลลงในหน้าหลัก</p>
            </div>
        ...
        <!-- Form Section -->
        <div
            class="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-2xl overflow-hidden backdrop-blur-xl">
            <form action="{{ route('backend.announcement.update', $announcement) }}" method="POST"
                enctype="multipart/form-data" class="p-8 sm:p-12 space-y-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div class="md:col-span-2 space-y-3">
                        <label for="title"
                            class="text-sm font-black text-zinc-700 dark:text-zinc-300 flex items-center gap-2 px-1">
                    ...
                    <div class="space-y-3">
                        ...
                        @if($announcement->image_path)
                            <div class="mt-4 relative group w-48">
                                <img src="{{ asset($announcement->image_path) }}" alt="Preview"
                                    class="rounded-xl shadow-lg border border-zinc-200 dark:border-zinc-800">
                                <div
                    ...
                    <div class="md:col-span-2 space-y-3">
                        ...
                        <textarea name="content" id="content" rows="6"
                            placeholder="กรอกรายละเอียดประกาศที่ต้องการแจ้งให้พนักงานทราบ..."
                    ...
                    <!-- Urgent Status -->
                    <div
                        class="md:col-span-2 bg-zinc-50 dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-100 dark:border-zinc-800 flex items-center justify-between group transition-all hover:bg-amber-50/20 dark:hover:bg-amber-900/10">
                        <div class="flex items-center gap-4">
                            <div
                                class="w-12 h-12 rounded-xl bg-white dark:bg-zinc-800 flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform">
                                <i class="fa-solid fa-triangle-exclamation text-amber-500 group-hover:animate-pulse"></i>
                            </div>
                            <div>
                                <span
                                    class="block text-sm font-black text-zinc-900 dark:text-white tracking-wide">เร่งด่วน</span>
                                <span class="text-xs text-zinc-500 font-bold">หากเป็นประกาศเร่งด่วน
                                    จะมีการเน้นสีให้เด่นชัดกว่าปกติ</span>
                            </div>
                        </div>
                        <label class="relative inline-flex items-center cursor-pointer">
                            <input type="checkbox" name="is_urgent" class="sr-only peer" {{ $announcement->is_urgent ? 'checked' : '' }} value="1">
                            <div
                                class="w-17 h-7 bg-zinc-200 peer-focus:outline-none dark:bg-zinc-800 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[4px] after:left-[4px] after:bg-white after:border-zinc-300 after:border after:rounded-full after:h-[20px] after:w-[20px] after:transition-all peer-checked:bg-amber-500">
                            </div>
                        </label>
                    </div>
                </div>
                ...
                <!-- Styled Form Footer -->
                <div
                    class="mt-12 -mx-8 sm:-mx-12 px-8 sm:px-12 py-10 bg-zinc-50 dark:bg-zinc-900 border-t border-zinc-100 dark:border-zinc-800">
                    <button type="submit"
                        class="w-full h-16 bg-amber-600 hover:bg-amber-700 text-white font-black rounded-2xl shadow-lg shadow-amber-900/10 active:scale-[0.98] transition-all flex items-center justify-center gap-3 text-lg uppercase tracking-wide">
                        <i class="fa-solid fa-save text-xl"></i>
                        <span>อัปเดตข้อมูลประกาศ</span>
                    </button>
                </div>
            </form>
        </div>
