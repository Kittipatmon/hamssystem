...
<div class="max-w-4xl mx-auto px-4 py-12">
    ...
    <div class="mb-10 text-center sm:text-left flex flex-col sm:flex-row justify-between items-center bg-white dark:bg-zinc-950 p-6 rounded-2xl border border-zinc-100 dark:border-zinc-800 shadow-sm gap-4 transition-all">
        <div>
            <h1 class="text-3xl font-black text-zinc-900 dark:text-white flex items-center justify-center sm:justify-start gap-4">
                <div class="p-2.5 bg-red-600 rounded-xl shadow-xl shadow-red-500/30">
                    <i class="fa-solid fa-bullhorn text-white text-xl"></i>
                </div>
                <span>เพิ่มประกาศใหม่</span>
            </h1>
            <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-2 font-medium">กรอกข้อมูลด้านล่างเพื่อสร้างประกาศใหม่ลงในหน้าหลัก</p>
        </div>
    ...
    <div class="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-3xl shadow-2xl overflow-hidden backdrop-blur-xl">
        <form action="{{ route('backend.announcement.store') }}" method="POST" enctype="multipart/form-data" class="p-8 sm:p-12 space-y-8">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="md:col-span-2 space-y-3">
                    <label for="title" class="text-sm font-black text-zinc-700 dark:text-zinc-300 flex items-center gap-2 px-1">
                        <i class="fa-solid fa-heading text-red-600 text-xs"></i>
                    ...
                    <input type="text" name="title" id="title" required placeholder="เช่น แจ้งปิดปรับปรุงระบบชั่วคราว..."
                        class="w-full bg-zinc-50 dark:bg-zinc-900 border-2 border-zinc-100 dark:border-zinc-800 focus:border-red-500 dark:focus:border-red-500 focus:ring-4 focus:ring-red-500/10 rounded-2xl px-5 py-4 transition-all duration-300 font-bold placeholder:text-zinc-400 dark:text-white"
                        value="{{ old('title') }}">
                </div>
                ...
                <div class="space-y-3">
                    <label for="image" class="text-sm font-black text-zinc-700 dark:text-zinc-300 flex items-center gap-2 px-1">
                        <i class="fa-solid fa-image text-red-600 text-xs"></i>
                        รูปภาพประกอบ
                    </label>
                    <input type="file" name="image" id="image" 
                        class="w-full bg-zinc-50 dark:bg-zinc-900 border-2 border-zinc-100 dark:border-zinc-800 focus:border-red-500 dark:focus:border-red-500 focus:ring-4 focus:ring-red-500/10 rounded-2xl px-5 py-3.5 transition-all duration-300 font-bold dark:text-white cursor-pointer file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-red-50 file:text-red-700 hover:file:bg-red-100">
                </div>

                <!-- Published Date -->
                <div class="space-y-3">
                    <label for="published_date" class="text-sm font-black text-zinc-700 dark:text-zinc-300 flex items-center gap-2 px-1">
                        <i class="fa-solid fa-calendar-check text-red-600 text-xs"></i>
                        วันที่ประกาศ
                    </label>
                    <input type="date" name="published_date" id="published_date" required
                        class="w-full bg-zinc-50 dark:bg-zinc-900 border-2 border-zinc-100 dark:border-zinc-800 focus:border-red-500 dark:focus:border-red-500 focus:ring-4 focus:ring-red-500/10 rounded-2xl px-5 py-4 transition-all duration-300 font-bold dark:text-white cursor-pointer"
                ...
                <div class="md:col-span-2 space-y-3">
                    <label for="content" class="text-sm font-black text-zinc-700 dark:text-zinc-300 flex items-center gap-2 px-1">
                        <i class="fa-solid fa-align-left text-red-600 text-xs"></i>
                        เนื้อหาประกาศ
                    </label>
                    <textarea name="content" id="content" rows="6" placeholder="กรอกรายละเอียดประกาศที่ต้องการแจ้งให้พนักงานทราบ..."
                        class="w-full bg-zinc-50 dark:bg-zinc-900 border-2 border-zinc-100 dark:border-zinc-800 focus:border-red-500 dark:focus:border-red-500 focus:ring-4 focus:ring-red-500/10 rounded-2xl px-5 py-4 transition-all duration-300 font-bold placeholder:text-zinc-400 dark:text-white resize-none">{{ old('content') }}</textarea>
                </div>

                <!-- Urgent Status -->
                <div class="md:col-span-2 bg-zinc-50 dark:bg-zinc-900 p-6 rounded-2xl border border-zinc-100 dark:border-zinc-800 flex items-center justify-between group transition-all hover:bg-red-50/20 dark:hover:bg-red-900/10">
