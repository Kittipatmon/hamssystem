...
@section('content')
	<div class="max-w-8xl mx-auto">
		<div class="mb-6 rounded-xl px-6 py-6 shadow-lg relative overflow-hidden">
			<div
				class="absolute inset-0 opacity-20 mix-blend-overlay bg-[url('https://www.transparenttextures.com/patterns/asfalt-dark.png')]">
			</div>
			<div class="relative z-10 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
				...
				<div class="w-full md:w-80">
					...
					<div class="relative group">
						<div
							class="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none text-red-400 group-focus-within:text-red-600 transition-colors">
							<i class="fa-solid fa-magnifying-glass"></i>
						</div>
						<input id="news-search" name="search" type="text" placeholder="ค้นหาข่าวสาร..."
							class="w-full rounded-full bg-white border border-red-100 focus:border-red-400 focus:ring-4 focus:ring-red-500/10 pl-10 pr-4 py-2.5 text-[14px] text-slate-700 shadow-sm transition-all duration-300 placeholder:text-slate-400 hover:border-red-200"
			...
			document.addEventListener('DOMContentLoaded', function () {
				...
				searchInput.addEventListener('input', function () {
					...
					timeoutId = setTimeout(() => {
						fetch(`{{ route('datamanage.news.newsalllist') }}?search=${encodeURIComponent(query)}`, {
							headers: {
							...
							.then(html => {
								// Parse the returned HTML to extract just the #news-container
								const parser = new DOMParser();
								const doc = parser.parseFromString(html, 'text/html');
								const newContent = doc.getElementById('news-container');
