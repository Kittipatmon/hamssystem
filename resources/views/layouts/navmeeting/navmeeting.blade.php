...
<nav
    ...
    <div class="max-w-7xl mx-auto px-4 md:px-6">
        <div class="h-16 flex items-center justify-between">
            ...
            <div class="hidden lg:flex items-center gap-2 lg:gap-3">
                ...
                <a href="{{ route('reservations.welcomemeeting') }}"
                    class="flex items-center gap-2 px-4 py-2 text-[14px] font-semibold rounded-full transition-all duration-300 {{ request()->routeIs('reservations.*') ? 'bg-red-600 text-white shadow-md shadow-red-200' : 'text-slate-600 hover:bg-red-50 hover:text-red-600' }}">
                    <i
                        class="fa-solid fa-calendar-check {{ request()->routeIs('reservations.*') ? '' : 'text-slate-400 group-hover:text-red-500' }}"></i>
                    <span>จองห้อง</span>
                </a>
                <a href="{{ route('rooms.index') }}"
                    class="flex items-center gap-2 px-4 py-2 text-[14px] font-semibold rounded-full transition-all duration-300 {{ request()->routeIs('rooms.*') ? 'bg-red-600 text-white shadow-md shadow-red-200' : 'text-slate-600 hover:bg-red-50 hover:text-red-600' }}">
                    <i
                        class="fa-solid fa-calendar-check {{ request()->routeIs('rooms.*') ? '' : 'text-slate-400 group-hover:text-red-500' }}"></i>
                        ...
                        <ul tabindex="0"
                            ...
                            <li>
                                <a href="{{ route('backend.bookingmeeting.rooms.index') }}"
                                    class="flex items-center gap-3 px-4 py-2.5 text-[14px] font-medium rounded-xl transition-colors {{ request()->routeIs('backend.bookingmeeting.rooms.*') ? 'bg-red-50 text-red-600' : 'text-slate-600 hover:text-red-600 hover:bg-red-50' }}">
                                    <i
                                        class="fa-solid fa-sliders w-4 text-center {{ request()->routeIs('backend.bookingmeeting.rooms.*') ? 'text-red-600' : 'text-red-400' }}"></i>
                            ...
                            <li>
                                <a href="{{ route('backend.bookingmeeting.report.index') }}"
                                    class="flex items-center gap-3 px-4 py-2.5 text-[14px] font-medium rounded-xl transition-colors {{ request()->routeIs('backend.bookingmeeting.report.*') ? 'bg-red-50 text-red-600' : 'text-slate-600 hover:text-red-600 hover:bg-red-50' }}">
                                    <i
                                        class="fa-solid fa-chart-pie w-4 text-center {{ request()->routeIs('backend.bookingmeeting.report.*') ? 'text-red-600' : 'text-red-400' }}"></i>
                                    รายงาน
                                </a>
                            </li>
                        </ul>
                ...
                @php
                    ...
                    if (Auth::check()) {
                        ...
                        // Meeting Room pending reservations
                    ...
                    <div tabindex="0" class="dropdown-content menu bg-white rounded-2xl mt-2 p-0 w-80 sm:w-96 shadow-2xl border border-slate-100 gap-0 animate-fadeIn z-[110] overflow-hidden">
                        ...
                        <div class="flex items-center justify-between px-4 py-3 bg-slate-50/80 border-b border-slate-100">
                            <div class="flex items-center gap-2">
                                <span class="font-bold text-slate-800 text-[15px]">การแจ้งเตือนห้องประชุม</span>
                            </div>
                        ...
                        <div class="p-3 bg-slate-50/50 border-t border-slate-100">
                            <button type="button" onclick="document.getElementById('all_notif_modal_meeting').showModal();" class="flex items-center justify-center w-full py-2.5 px-4 bg-red-600 hover:bg-red-700 text-white font-bold text-xs rounded-xl shadow-md shadow-red-200 transition-all cursor-pointer">
                ...
                <dialog id="all_notif_modal_meeting" class="modal modal-bottom sm:modal-middle z-[200]">
                    <div class="modal-box bg-white rounded-3xl p-0 max-w-2xl overflow-hidden shadow-2xl border border-slate-100 text-left">
                        <!-- Modal Header -->
                        <div class="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-purple-600 to-indigo-700 text-white">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 class="font-black text-lg leading-tight">การแจ้งเตือนจองห้องประชุม</h3>
                                    <p class="text-xs text-purple-100 font-medium">รวมรายการจองห้องประชุมและอนุมัติเวลาใช้งาน</p>
                                </div>
                            </div>
                            <form method="dialog">
                        ...
                        <div class="p-6 max-h-[60vh] overflow-y-auto divide-y divide-slate-100 custom-scrollbar">
                            @forelse($realNotifs as $notif)
                                <a href="{{ $notif['url'] }}" class="flex items-start gap-4 py-4 px-3 rounded-2xl hover:bg-purple-50/60 transition-all duration-200 group">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center justify-between gap-2">
                                            <h4 class="text-xs font-bold text-slate-800 group-hover:text-purple-600 transition-colors truncate">{{ $notif['title'] }}</h4>
                                        ...
                                        <div class="mt-2 inline-flex items-center gap-1 text-[11px] font-bold text-purple-600 group-hover:translate-x-1 transition-transform">
                            ...
                            @empty
                                <div class="py-12 text-center">
                                    ...
                                    <p class="text-xs text-slate-400 mt-1">ทุกรายการคำขอจองห้องประชุมถูกอนุมัติเรียบร้อยแล้ว</p>
                                </div>
                            @endforelse
                        </div>
        ...
        <!-- Mobile nav -->
        <div id="mnav"
            class="lg:hidden hidden pb-4 pt-2 border-t border-slate-100 animate-fadeIn max-h-[75vh] overflow-y-auto custom-scrollbar">
                ...
                @if($isHamsOrAdmin)
                    <details class="group [&_summary::-webkit-details-marker]:hidden" {{ $isReportActive ? 'open' : '' }}>
                        ...
                        <div class="mt-1 mb-2 ml-4 pl-4 border-l-2 border-red-100 flex flex-col gap-1">
                            <a href="{{ route('backend.bookingmeeting.rooms.index') }}"
                                class="flex items-center gap-3 px-4 py-2.5 text-[14px] font-medium rounded-lg transition-colors {{ request()->routeIs('backend.bookingmeeting.rooms.*') ? 'bg-red-50 text-red-600 font-bold' : 'text-slate-600 hover:bg-red-50 hover:text-red-600' }}">
                                <i
                                    class="fa-solid fa-sliders w-4 text-center {{ request()->routeIs('backend.bookingmeeting.rooms.*') ? 'text-red-600' : 'text-red-400' }}"></i>
                                จัดการข้อมูลห้องประชุม
                            </a>
